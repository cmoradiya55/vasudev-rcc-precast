...
const ProhibitedItem = ({ text }: { text: string }) => (
  <li className="flex items-start gap-3 group">
    <XCircle className="w-4 h-4 mt-0.5 text-red-400 shrink-0 group-hover:scale-110 transition-transform" />
    <span className="text-gray-600 leading-relaxed">{text}</span>
  </li>
);

const Terms = () => {
  const { content } = termsData as { content: Record<string, unknown> };
  const sections = Object.entries(content)
    .filter(([key]) => key !== "intro")
    .map(([key, value]) => {
      const section = value as TermsSection;
      return {
        id: key,
        title: section.title,
        icon: section.icon,
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-16">
            ...
            >
              <div className="lg:sticky lg:top-28">
                ...
                {/* Side trust badge — hidden on mobile */}
            ...
            <article className="flex-1 max-w-3xl pb-12 lg:py-12 space-y-6 text-[14px] lg:text-[15px]">
              ...
              {/* Sections from JSON */}
              {sections.map((s) => {
                const sectionContent = content[s.id] as TermsSection;
                ...
                return (
                  <SectionCard key={s.id} id={s.id} title={s.title} icon={Icon}>
                    <div className="space-y-4">
                      ...
                      {sectionContent.prohibited && (
                        <ul className="mt-4 space-y-2.5">
                          {sectionContent.prohibited.map((item: string) => (
                            <ProhibitedItem key={item} text={item} />
                          ))}
                        </ul>
                      )}
                      ...
                      {/* Special link for privacy in privacy section */}
                      {s.id === "privacy" && (
                        <div className="mt-2">
                          <Link
                            href="/privacy"
